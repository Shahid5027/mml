...
export const getInsightsFeed = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    // 2.5 Fetch recent device trust alerts (unrecognized device logins)
    const deviceAlertsQuery = `
      SELECT 
        'DEVICE_ALERT' as category,
        u.name,
        u.email,
        82 as "score",
        'New Device' as "status",
        0 as "failedCount",
        da.timestamp,
        da.details
      ...
      LIMIT 15
    `;
    const deviceAlertsRes = await pool.query(deviceAlertsQuery);

    // Count distinct unrecognized device fingerprints seen today
    const newDevicesTodayQuery = `
      SELECT COUNT(DISTINCT fingerprint) as new_count
      FROM device_alerts
      WHERE timestamp >= CURRENT_DATE
    `;
    const newDevicesTodayRes = await pool.query(newDevicesTodayQuery);
    const newDevicesToday = parseInt(newDevicesTodayRes.rows[0].new_count) || 0;
