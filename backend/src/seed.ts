...
const seedDatabase = async () => {
  ...
  try {
    ...
    await pool.query('TRUNCATE office_settings, attendance_insights, attendance_logs, attendance, trusted_devices, device_alerts, users CASCADE;');
    ...
    // Seed trusted devices and device login alerts
    console.log('📌 Seeding trusted devices and device login alerts...');
    const alexId = empIds['employee1@geoshield.ai'];
    const sarahId = empIds['employee2@geoshield.ai'];
    const marcusId = empIds['employee3@geoshield.ai'];
    
    await pool.query(`
      INSERT INTO trusted_devices (user_id, fingerprint, device_name, is_trusted)
      VALUES 
        ('${alexId}', 'dev_alex_1', 'Chrome on Windows PC', true),
        ('${sarahId}', 'dev_sarah_1', 'Safari on MacBook', true)
    `);

    await pool.query(`
      INSERT INTO device_alerts (user_id, fingerprint, device_name, event_type, details, timestamp)
      VALUES 
        ('${marcusId}', 'dev_marcus_new', 'Chrome on Linux Machine', 'NEW_DEVICE_LOGIN', 'Marcus Wright logged in from an unrecognized device: Chrome on Linux Machine.', NOW() - INTERVAL '3 HOURS'),
        ('${alexId}', 'dev_alex_new', 'Firefox on iOS Device', 'NEW_DEVICE_LOGIN', 'Alex Mercer logged in from an unrecognized device: Firefox on iOS Device.', NOW() - INTERVAL '5 HOURS')
    `);
