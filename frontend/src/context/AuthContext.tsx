...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, password: string): Promise<User> => {
    try {
      ...
      const { token: receivedToken, user: receivedUser, isNewDevice } = response.data;

      localStorage.setItem('geoshield_token', receivedToken);
      localStorage.setItem('geoshield_user', JSON.stringify(receivedUser));
      localStorage.setItem('geoshield_is_new_device', isNewDevice ? 'true' : 'false');
  ...
  const logout = () => {
    ...
    localStorage.removeItem('geoshield_is_new_device');
    setToken(null);
    setUser(null);
  };
