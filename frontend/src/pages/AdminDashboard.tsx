...
const MOCK_INSIGHTS_FEED = [
  ...
  },
  {
    ...
    details: 'Marcus Wright logged in from an unrecognized device: Chrome on Linux Machine.'
  },
  {
    category: 'DEVICE_ALERT',
    name: 'Alex Mercer',
    ...
    status: 'New Device',
    failedCount: 0,
    timestamp: '2026-05-18T08:45:10Z',
    details: 'Alex Mercer logged in from an unrecognized device: Firefox on iOS Device.'
  }
];
...
const AdminDashboard: React.FC = () => {
  ...
  // Tab State
  const [activeTab, setActiveTab] = useState<'roster' | 'insights' | 'settings' | 'employees'>(() => {
    const searchParams = new URLSearchParams(location.search);
    const tabParam = searchParams.get('tab');
    if (tabParam === 'roster' || tabParam === 'insights' || tabParam === 'settings' || tabParam === 'employees') {
      return tabParam;
    }
    return 'roster';
  });
  ...
  useEffect(() => {
    const searchParams = new URLSearchParams(location.search);
    const tabParam = searchParams.get('tab');
    if (tabParam && (tabParam === 'roster' || tabParam === 'insights' || tabParam === 'settings' || tabParam === 'employees')) {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setInsightsStats({
        ...
        newDevicesCount: insightsRes.data.newDevicesToday !== undefined 
          ? insightsRes.data.newDevicesToday 
          : feed.filter((f: any) => f.category === 'DEVICE_ALERT').length,
      });
    } catch (err: any) {
      ...
      setInsightsStats({
        ...
        newDevicesCount: MOCK_INSIGHTS_FEED.filter(f => f.category === 'DEVICE_ALERT').length,
      });
    } finally {
      setLoading(false);
    }
          ...
          {activeTab === 'insights' && (
            ...
            >
              ...
              <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {/* Insights Card 3 */}
                <div className="glass-card p-5 rounded-2xl shadow-premium flex items-center gap-4">
                  <div className="p-3.5 bg-zinc-800/85 border border-white/[0.06] text-zinc-400 rounded-xl shadow-inner">
                ...
                {/* Insights Card 4: Device Trust */}
                <div className="glass-card p-5 rounded-2xl shadow-premium flex items-center gap-4 animate-premium">
                  <div className="p-3.5 bg-amber-500/10 border border-amber-500/20 text-amber-400 rounded-xl shadow-inner">
                    <Laptop className="w-6 h-6" />
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-0.5 font-sans">New Device Logins</span>
                    <span className="text-2xl font-extrabold text-white leading-none font-mono">
                      {loading ? '--' : <AnimatedCounter value={insightsStats.newDevicesCount} />}
                    </span>
                    <p className="text-[10px] text-zinc-500 mt-1 font-semibold">Unrecognized fingerprints today</p>
                  </div>
                </div>
              </section>
                      ...
                      <div key={index} className="relative group">
                        ...
                        <div className="bg-[#09090b]/40 border border-white/[0.04] p-4 rounded-xl flex flex-col sm:flex-row items-start justify-between gap-4 group-hover:border-white/[0.08] transition-colors duration-200">
                          ...
                          {item.category === 'SUCCESS' ? (
                            ...
                            <div className="flex flex-col items-end shrink-0 sm:text-right">
                              <span className="inline-flex items-center gap-1 text-[10px] font-extrabold px-2.5 py-1 rounded-full border bg-amber-500/10 border-amber-500/20 text-amber-400">
                                NEW DEVICE LOGIN
                              </span>
                              <span className="text-[9px] font-bold uppercase tracking-wider text-amber-500 mt-1 flex items-center gap-1">
                                <Laptop className="w-3 h-3 text-amber-400 shrink-0" /> UNFAMILIAR
                              </span>
                            </div>
                          ) : (
                            <div className="flex flex-col items-end shrink-0 sm:text-right">
